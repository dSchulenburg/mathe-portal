...
import { exercises } from '../data/exercises/10-koerper';
describe('Körper exercises', () => {
  it('has 15 exercises', () => { expect(exercises.length).toBe(15); });
  it('all have required fields', () => { exercises.forEach(ex => { expect(ex.id).toBeTruthy(); expect(ex.topicId).toBe('10-koerper'); expect(DIFF_LEVELS).toContain(ex.diffLevel); expect(ex.competencies.length).toBeGreaterThan(0); expect(ex.data).toBeDefined(); expect(ex.solutionKey).toBeTruthy(); }); });
