export const batch12b = {
  '12-normalverteilung': {
    ...
    objectives: {
      ...
      sigma_rules: 'Die Sigma-Regeln ($68{,}3\\%$, $95{,}4\\%$, $99{,}7\\%$) für schnelle Wahrscheinlichkeitsabschätzungen anwenden',
    ...
    explanation: {
      intro: 'Die Normalverteilung ist die wichtigste stetige Verteilung der Statistik — und sie begegnet dir überall: bei Körpergrößen, Messwerten, Frame-Zeiten, Prüfungsergebnissen. Ihre Dichte hat die ikonische Glockenform, symmetrisch um den Erwartungswert $\\mu$. Die Standardabweichung $\\sigma$ bestimmt, wie breit oder schmal die Glocke ist: kleines $\\sigma$ = schmale, hohe Glocke (wenig Streuung), großes $\\sigma$ = breite, flache Glocke (viel Streuung). Die Dichtefunktion lautet:',
      ...
      kai_tip: 'In der Game-Entwicklung nutze ich die Sigma-Regeln beim Balancing täglich. Wenn ich will, dass 95% der Loot-Drops zwischen 30 und 70 Gold liegen, setze ich $\\mu = 50$ und $2\\sigma = 20$, also $\\sigma = 10$. Fertig! Die Regeln geben mir sofort die Verteilung, ohne dass ich irgendetwas integrieren muss.',
      ...
      central_limit: 'Warum ist die Normalverteilung so allgegenwärtig? Der Zentrale Grenzwertsatz gibt die Antwort: Wenn du genügend viele unabhängige Zufallsgrößen addierst, nähert sich ihre Summe — egal wie die Einzelverteilungen aussehen — einer Normalverteilung an. Deshalb sind Mittelwerte fast immer normalverteilt, selbst wenn die Einzeldaten es nicht sind:',
    },
    concepts: {
      normal_distribution: {
        title: 'Normalverteilung $N(\\mu, \\sigma^2)$',
        desc: 'Die stetige Verteilung mit glockenförmiger Dichte. $\\mu$ ist der Erwartungswert (Mittelpunkt der Glocke), $\\sigma^2$ die Varianz (Breite der Glocke). Die Fläche unter der Kurve ist immer 1. Symmetrisch um $\\mu$: Median = Modus = Erwartungswert.',
      },
      sigma_rules: {
        title: 'Sigma-Regeln',
    ...
    examples: {
      loot_balancing: {
        ...
        context: 'In "Hafenlichter 3D" droppen Bosse Gold, normalverteilt mit $\\mu = 50$ und $\\sigma = 8$. Kai will wissen: Wie wahrscheinlich ist ein legendärer Drop von mindestens 70 Gold?',
        ...
        kai_comment: 'Perfekt! Ein legendärer Drop bei weniger als 1% der Spieler — das fühlt sich besonders an, ohne unfair zu sein. Die Normalverteilung gibt mir die totale Kontrolle über die Seltenheit.',
      },
      frame_times: {
    ...
    realworld: {
      ...
      iq_scores: {
        ...
        desc: 'IQ-Werte sind per Definition normalverteilt mit $\\mu = 100$ und $\\sigma = 15$. Das bedeutet: Ca. $68\\%$ der Bevölkerung haben einen IQ zwischen 85 und 115, ca. $95\\%$ zwischen 70 und 130. Ein IQ von 145 ($z = 3$) ist so selten wie ein 70-Gold-Loot-Drop!',
      },
      stock_returns: {
        title: 'Aktienrenditen und Risiko',
        desc: 'Tägliche Aktienrenditen werden oft als normalverteilt modelliert — $\\sigma$ ist dann das Risiko. Portfoliomanager nutzen die $z$-Transformation, um zu berechnen, wie wahrscheinlich ein Crash ist. Allerdings: In der Realität kommen extreme Ausreisser häufiger vor als die Normalverteilung vorhersagt ("Fat Tails").',
      },
    },
    mistakes: {
      sigma_vs_variance: {
        ...
        kai_warning: 'Dieser Fehler hat mir mal das komplette Loot-Balancing zerstört. Ich hatte $\\sigma = 8$ gemeint, aber $N(50, 8)$ geschrieben — das bedeutet $\\sigma = \\sqrt{8} \\approx 2{,}83$. Die Drops waren viel zu gleichförmig! Immer dran denken: $N(\\mu, \\sigma^2)$!',
      },
      z_sign: {
  ...
  '12-matrizen': {
    ...
    concepts: {
      ...
      transformation_matrix: {
        ...
        desc: 'Jede lineare Abbildung (Drehung, Skalierung, Spiegelung) lässt sich als Matrix darstellen. Die Rotationsmatrix $R(\\theta)$ dreht Vektoren um den Winkel $\\theta$. Mehrere Transformationen werden durch Matrizenmultiplikation verkettet — von rechts nach links gelesen.',
      },
      stochastic_matrix: {
    ...
    examples: {
      '3d_rotation': {
        ...
        step4: 'Ergebnis: Die Laterne sitzt jetzt bei $(0, 3, 5)$ — um $90°$ in der $xy$-Ebene gedreht, $z$ unverändert:',
      ...
      player_states: {
        ...
        context: 'In "Hafenlichter 3D" wechseln Spieler zwischen Erkunden (E) und Kämpfen (K). Kai hat beobachtet: Wer erkundet, bleibt zu $70\\%$ beim Erkunden und wechselt zu $30\\%$ zum Kampf. Wer kämpft, bleibt zu $80\\%$ im Kampf und erkundet zu $20\\%$ wieder. Aktuell erkunden $60\\%$ der Spieler.',
        ...
        step4: 'Langfristig erkunden $40\\%$ und kämpfen $60\\%$ — unabhängig vom Start:',
        kai_comment: 'Das ist Gold für mein Game-Design! Der Fixvektor zeigt mir, dass die Spieler langfristig mehr kämpfen als erkunden — egal wie sie starten. Wenn ich will, dass es ausgeglichener ist, muss ich die Übergangswahrscheinlichkeiten ändern. Matrizen machen Spielerverhalten berechenbar!',
      },
    },
    realworld: {
      computer_graphics: {
      ...
      google_pagerank: {
        ...
        desc: 'Googles ursprünglicher Suchalgorithmus modelliert das Web als riesige stochastische Matrix: Jede Webseite ist ein Zustand, jeder Link eine Übergangswahrscheinlichkeit. Der Fixvektor $\\vec{r} = T \\cdot \\vec{r}$ ergibt das Ranking — die Seiten mit den höchsten Werten im Fixvektor stehen ganz oben.',
      },
      robotics: {
        title: 'Robotik & Maschinensteuerung',
        desc: 'Roboterarme bestehen aus Gelenken, die jeweils eine Rotation ausführen. Die Gesamtposition der Hand ergibt sich als Produkt aller Rotationsmatrizen entlang des Arms. Das nennt man Vorwärtskinematik — und die inverse Matrix löst das Rückwärtsproblem: "Welche Gelenkwinkel brauche ich für diese Handposition?"',
      },
    },
    mistakes: {
      commutative: {
        ...
        why: 'Erst drehen, dann verschieben ergibt ein anderes Ergebnis als erst verschieben, dann drehen. In der 3D-Grafik führt vertauschte Reihenfolge zu völlig falschen Positionen. Deshalb: Transformationsketten immer von rechts nach links lesen!',
      ...
      stochastic_rows_cols: {
        ...
        why: 'Die Konvention hängt davon ab, ob du Zeilen- oder Spaltenvektoren verwendest. In der Schule und an deutschen Unis ist der Spaltenvektor Standard — dann müssen die Spalten der Übergangsmatrix die Summe 1 haben. Jede Spalte beschreibt: "Von Zustand $j$ aus — wie verteilen sich die Übergänge?"',
      },
    },
  },
  ...
  '12-abiturvorbereitung': {
    ...
    objectives: {
      exam_strategy: 'Eine klare Prüfungsstrategie entwickeln: Aufgaben sichten, Punkte-Zeit-Verhältnis berechnen, leichte Aufgaben zuerst',
    ...
    explanation: {
      ...
      time_strategy: 'Plane deine Zeit proportional zu den Punkten. Wenn eine Aufgabe 10 von 100 Punkten bringt, investiere ca. $10\\%$ deiner Zeit. Und die goldene Regel: Beginne mit den Aufgaben, bei denen du dir sicher bist. Jeder sichere Punkt zählt gleich viel wie ein schwer erkämpfter!',
    ...
    concepts: {
      ...
      point_maximizing: {
        ...
        desc: 'Nicht jede Aufgabe ist gleich schwer pro Punkt. Beginne mit den Aufgaben, bei denen du die meisten Punkte in der kürzesten Zeit holst. Die letzten Teilaufgaben sind oft am schwersten — lieber erstmal alle "leichten" Punkte in allen Aufgaben sammeln, dann zu den schweren zurückkehren.',
      },
      plausibility_check: {
        title: 'Plausibilitäts-Check',
        desc: 'Vier schnelle Checks, die Flüchtigkeitsfehler verhindern: (1) Einheiten: Passt die Einheit zum Kontext? (2) Vorzeichen: Ist negativ hier sinnvoll? (3) Grenzwerte: Was passiert für $x \\to 0$ oder $x \\to \\infty$? (4) Skizze: Passt das Ergebnis zur grafischen Vorstellung?',
      },
    },
    examples: {
      mixed_analysis: {
        title: 'Typische Analysisaufgabe: Kurvendiskussion + Integral',
        context: 'Kai modelliert den Energieverbrauch seiner 3D-Engine mit $f(x) = (2x - 1) \\cdot e^{-x}$. Bestimme Extremstellen und berechne die Fläche zwischen Graph und $x$-Achse auf $[0;\\, 3]$.',
        ...
        step4: 'Flächenberechnung — partielle Integration oder Stammfunktion bestimmen:',
        mia_comment: 'Das ist genau das Muster, das ich meine: Ableitung mit Produktregel, Nullstelle finden, Integral berechnen. Die Funktion ändert sich, aber die Strategie bleibt immer gleich. Üben, bis es automatisch läuft!',
      },
      mixed_stochastik: {
    ...
    realworld: {
      release_day: {
        ...
        desc: 'Ein Spiel zu veröffentlichen ist wie eine Prüfung: Monate der Vorbereitung münden in einen entscheidenden Moment. Kais "Hafenlichter 3D" vereint 2D-Grundlagen (Mia), Datenanalyse (Amir) und 3D-Mathematik (Kai) — genau wie das Abitur Analysis, Geometrie und Stochastik vereint. Vorbereitung ist alles.',
      },
      project_management: {
      ...
      lifelong_learning: {
        ...
        desc: 'Das Abitur ist nicht das Ende, sondern der Anfang. Mia studiert Game-Design, Amir Data Science, Kai Medieninformatik — und überall brauchen sie Mathematik. Die Fähigkeit, sich in neue Themen einzuarbeiten, ist wertvoller als jede einzelne Formel. Mathe hat euch das Denken beigebracht.',
      },
    },
    mistakes: {
      no_units_context: {
        ...
        why: 'Im Abitur gibt es Punkte für die Interpretation im Sachkontext! Ein nacktes Ergebnis ohne Einheit oder Bezug zur Aufgabenstellung kostet dich leichte Punkte. Schreibe immer einen Antwortsatz: "Die Fläche beträgt $12{,}5\\,\\text{m}^2$, was der begrünten Fläche des Parks entspricht."',
      ...
      skip_plausibility: {
        ...
        why: 'Plausibilitätschecks kosten 10 Sekunden, können aber ganze Aufgaben retten. Wahrscheinlichkeiten liegen immer zwischen 0 und 1, Flächeninhalte sind nie negativ, $e^x > 0$ für alle $x$. Wenn dein Ergebnis gegen diese Grundregeln verstößt, steckt ein Rechenfehler dahinter.',
