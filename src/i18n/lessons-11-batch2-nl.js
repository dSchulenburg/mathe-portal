export const batch11b = {
  '11-exp-log': {
    story: {
      intro: 'Amirs muziekaanbevelingsalgoritme heeft een probleem: de populariteit van nieuwe songs stijgt aanvankelijk explosief — maar hoe snel precies? En wanneer vlakt de hype af? "Ik heb een functie nodig die extreem snelle groei beschrijft en toch berekenbaar blijft", zegt Amir tegen Yara. Het antwoord: de exponentiële functie met het getal van Euler $e$ — en haar tegenhanger, de natuurlijke logaritme.',
      challenge: 'Amir moet de groeicurve van songstreams modelleren en voorspellen wanneer een track zijn piek bereikt — daarvoor heeft hij exponentiële vergelijkingen en logaritmen nodig.',
      outro: 'Met de $e$-functie en $\\ln$ kan Amir nu berekenen wanneer een song zijn hoogtepunt bereikt en hoe snel de interesse daarna afneemt. "Het mooie is: de natuurlijke logaritme maakt elke exponentiële vergelijking oplosbaar", legt hij uit aan Finn. DataPulse voorspelt nu songtrends met verrassende precisie.',
    },
    objectives: {
      ...
      log_rules: 'Logaritmewetten veilig toepassen (product-, quotiënt- en machtsregel)',
      solve_exp_equations: 'Exponentiële vergelijkingen oplossen door logaritmiseren',
    },
    explanation: {
      ...
      solving: 'Om een exponentiële vergelijking naar $x$ op te lossen, pas je aan beide kanten de $\\ln$ toe. De logaritme "haalt de exponent naar beneden" — dat is de sleutelstap:',
    },
    concepts: {
      ...
      log_rules: { title: 'Logaritmewetten', desc: 'Drie regels die alles vereenvoudigen: productregel ($\\ln(a \\cdot b) = \\ln(a) + \\ln(b)$), quotiëntregel ($\\ln(\\frac{a}{b}) = \\ln(a) - \\ln(b)$) en machtsregel ($\\ln(a^r) = r \\cdot \\ln(a)$). De machtsregel is de sleutel tot het oplossen van exponentiële vergelijkingen.' },
    },
    examples: {
      solve_exp: { title: 'Exponentiële vergelijking oplossen', context: 'Amir wil berekenen na hoeveel dagen een song 5 keer zoveel streams heeft als in het begin. Het groeimodel luidt: $S(x) = 3 \\cdot e^{2x}$.', step1: 'Vergelijking opstellen — wanneer bereiken de streams de waarde 15?', step2: 'Door 3 delen om $e^{2x}$ te isoleren:', step3: 'Aan beide kanten de $\\ln$ toepassen — de exponent "komt naar beneden":', step4: 'Naar $x$ oplossen — delen door de coëfficiënt 2:', amir_comment: 'Dit is het patroon dat ik constant nodig heb: isoleren, logaritmiseren, oplossen. Werkt bij elke exponentiële vergelijking!' },
    ...
    mistakes: {
      ...
      ln_cancel: { wrong: '$\\ln(e^x) = e \\cdot x$ — logaritme fout opgelost', correct: '$\\ln(e^x) = x$ — de logaritme heft de exponentiële functie op', why: '$\\ln$ en $e^x$ zijn omkeerfuncties — ze heffen elkaar op. Het resultaat is gewoon de exponent $x$, zonder factor $e$ ervoor.' },
    },
  },
  ...
  '11-binomialverteilung': {
    ...
    explanation: {
      ...
      bernoulli: 'De basis is het Bernoulli-experiment — een enkel experiment met precies twee uitkomsten. Drie voorwaarden moeten gelden: (1) slechts twee resultaten, (2) de kans $p$ blijft bij elke poging gelijk, (3) de pogingen zijn onafhankelijk. De binomiaalcoëfficiënt telt de mogelijke schikkingen:',
      amir_tip: 'De binomiaalcoëfficiënt $\\binom{n}{k}$ telt op hoeveel manieren je $k$ successen over $n$ pogingen kunt verdelen. In mijn code is dat de functie scipy.special.comb(n, k). Voor grote $n$ bereken ik liever de cumulatieve verdeling — afzonderlijke kansen optellen zou veel te langzaam zijn!',
    ...
    realworld: {
      ...
      medicine: { title: 'Medische studies', desc: 'Bij medicijntests tel je: hoeveel patiënten worden beter? Is het genezingspercentage significant beter dan bij placebo? De binomiale verdeling levert het antwoord — en beschermt tegen voorbarige conclusies bij te kleine steekproeven.' },
      surveys: { title: 'Opiniepeilingen en fouttolerantie', desc: 'Als bij een enquête 52% "ja" stemt — is dat echt een meerderheid? De standaardafwijking $\\sigma = \\sqrt{n \\cdot p \\cdot (1-p)}$ toont hoe breed het betrouwbaarheidsinterval is. Bij $n = 100$ zou $\\sigma \\approx 5\\%$ zijn — het resultaat zou ook 47% of 57% kunnen zijn.' },
    },
    mistakes: {
      forget_binomial_coeff: { wrong: 'Binomiaalcoëfficiënt vergeten: $P(X=k) = p^k \\cdot (1-p)^{n-k}$', correct: 'Volledige formule met $\\binom{n}{k}$', why: 'Zonder $\\binom{n}{k}$ telt de formule slechts ÉÉN bepaalde volgorde — bijv. eerst $k$ successen, dan $n-k$ mislukkingen. Maar de successen kunnen op verschillende plaatsen voorkomen! De binomiaalcoëfficiënt telt alle mogelijke schikkingen.', amir_warning: 'Stel je voor, je gooit 3 keer en wilt precies 2 zessen. Er zijn 3 mogelijkheden: 6-6-X, 6-X-6, X-6-6. De binomiaalcoëfficiënt $\\binom{3}{2} = 3$ telt precies dat. Zonder hem krijg je slechts een derde van de juiste kans!' },
  ...
  '11-bedingte-wahrscheinlichkeit': {
    ...
    examples: {
      medical_test: { title: 'Medische test — Base Rate Fallacy', context: 'Een ziektetest herkent 99% van de zieken, maar slaat ook bij 5% van de gezonden aan. De ziekte treft 1% van de bevolking. Als de test positief is — hoe waarschijnlijk ben je echt ziek?', step1: 'Gegeven: $P(K) = 0{,}01$ (ziekte), $P(+ \\mid K) = 0{,}99$ (sensitiviteit), $P(+ \\mid \\bar{K}) = 0{,}05$ (fout-positief percentage).', step2: 'Totale kans op een positief resultaat berekenen — beide wegen (ziek + gezond):', step3: 'Stelling van Bayes toepassen — gezocht is $P(K \\mid +)$:', step4: 'Resultaat: slechts 16,7% kans om echt ziek te zijn — hoewel de test 99% van de zieken herkent!', amir_comment: 'Dat is de Base Rate Fallacy! De test is super nauwkeurig, maar omdat de ziekte zo zeldzaam is ($P(K) = 1\\%$), zijn de meeste positieve tests fout-positief. In de ML-wereld heet dat: Precision en Recall zijn niet hetzelfde!' },
    ...
    mistakes: {
      ...
      ignore_base_rate: { wrong: 'Het basispercentage genegeerd (Base Rate Fallacy)', correct: 'Altijd $P(A)$ — de prior-kans — meenemen', why: 'Een test die 99% van de zieken herkent, klinkt indrukwekkend, maar als de ziekte slechts 1% treft, is $P(K \\mid +) \\approx 16{,}7\\%$. Het lage basispercentage $P(K) = 0{,}01$ domineert het resultaat. Zonder de Prior krijg je een volstrekt vertekende inschatting.' },
    },
  },
