  ...
  '12-e-funktion-ln': {
    ...
    concepts: {
      ...
      ln_derivative: {
        ...
        desc: 'Die Kettenregel für den Logarithmus erzeugt einen Bruch: innere Ableitung durch innere Funktion. Das ist besonders nützlich, wenn $g(x)$ ein Polynom ist — z.B. $(\\ln(x^2+1))\' = \\frac{2x}{x^2+1}$. Wichtig: Der Definitionsbereich wird durch $g(x) > 0$ eingeschränkt.',
      },
      exp_integration: {
    ...
    examples: {
      ...
      ln_curve_analysis: {
        ...
        context: 'Kai modelliert die Helligkeit eines Lichtblitzes: Erst steigt sie an, dann fällt sie ab. Die Helligkeitskurve hat die Form $f(x) = x^2 \\cdot e^{-x}$ — eine typische "Aufblitz-Funktion".',
    ...
    realworld: {
      ...
      signal_processing: {
        title: 'Audioverarbeitung & Signaldämpfung',
        desc: 'Wenn du in GarageBand oder einem DJ-Tool einen Hall-Effekt hörst, klingt der Ton exponentiell ab: $S(t) = A \\cdot e^{-\\alpha t} \\cdot \\sin(\\omega t)$. Die $e$-Funktion steuert die Lautstärke, der $\\sin$ die Schwingung. Auch Bluetooth-Signale und WLAN-Reichweite folgen diesem Modell — deshalb wird das Streaming ruckelig, je weiter du vom Router weg bist.',
      },
      learning_curve: {
        title: 'Lernkurve & Skill-Progression',
        desc: 'Wie schnell lernst du ein neues Spiel? Die Lernkurve $L(t) = L_{\\max} \\cdot (1 - e^{-kt})$ beschreibt, wie Fähigkeiten anfangs schnell wachsen und sich dann einem Maximum nähern. Spieledesigner nutzen dieses Modell, um Schwierigkeitskurven zu designen — und auch dein Gehirn folgt beim Vokabellernen oder Instrumentüben genau dieser Funktion.',
      },
    },
    mistakes: {
  ...
  '12-integralrechnung-vertieft': {
    ...
    concepts: {
      partial_integration: {
        ...
        desc: 'Die Umkehrung der Produktregel: Du zerlegst den Integranden in $u$ (wird abgeleitet) und $\\mathrm{d}v$ (wird integriert). Ziel: Das neue Integral $\\int v\\,\\mathrm{d}u$ muss einfacher sein als das ursprüngliche. Typische Anwendung: Produkte aus Polynom und $e$-Funktion oder Polynom und Trigonometrie.',
      },
      rotation_volume: {
        title: 'Rotationsvolumen',
        desc: 'Ein Funktionsgraph, der um die $x$-Achse rotiert, erzeugt einen 3D-Körper. Das Volumen ergibt sich aus der Summe unendlich vieler Kreisscheiben mit Radius $f(x)$ und Dicke $\\mathrm{d}x$. Jede Scheibe hat das Volumen $\\pi \\cdot [f(x)]^2 \\cdot \\mathrm{d}x$ — aufsummiert (integriert) ergibt das die Formel.',
      },
      improper_integral: {
    ...
    examples: {
      level_geometry: {
        title: 'Rotationskörper für Level-Design',
        context: 'Kai will eine Säule generieren, deren Profil der Kurve $f(x) = \\sqrt{x}$ folgt. Zwischen $x = 0$ und $x = 4$ rotiert die Kurve um die $x$-Achse — wie groß ist das Volumen der Säule?',
        ...
        step4: 'Ergebnis — die Säule hat ein Volumen von $8\\pi$ Volumeneinheiten:',
      ...
      partial_int: {
        ...
        step1: 'Zerlegung wählen: $u = x$ (wird einfacher beim Ableiten), $v\' = e^x$ (lässt sich leicht integrieren):',
    ...
    realworld: {
      '3d_printing': {
        ...
        desc: 'Jeder 3D-Drucker muss wissen, wie viel Material er braucht. Bei rotationssymmetrischen Teilen (Vasen, Röhren, Düsen) berechnet die Slicer-Software das Volumen mit genau dieser Rotationsformel. Auch in CAD-Programmen wie Fusion 360 steckt Integralrechnung — wenn du einen Querschnitt rotierst, rechnet die Software $\\pi \\int [r(h)]^2\\,\\mathrm{d}h$.',
      },
      render_falloff: {
        title: 'Licht-Falloff in Game-Engines',
        desc: 'Wie viel Gesamtlicht empfängt eine Szene von einer Punktlichtquelle? Die Intensität nimmt mit $I(r) = I_0 \\cdot e^{-\\alpha r}$ ab. Das uneigentliche Integral $\\int_0^\\infty I_0 \\cdot e^{-\\alpha r}\\,\\mathrm{d}r = \\frac{I_0}{\\alpha}$ zeigt: Die Gesamtenergie ist endlich! Deshalb können Game-Engines Lichter mit begrenzter Reichweite simulieren, ohne physikalisch falsch zu liegen.',
      },
      terrain_volume: {
        title: 'Terrain-Generierung & Erdarbeiten',
        desc: 'Bei Strassenbauprojekten und in Minecraft-artigen Spielen muss das Volumen von Hügellandschaften berechnet werden. Die Formel $V = \\iint h(x,y)\\,\\mathrm{d}A$ summiert die Höhe über die gesamte Fläche auf — ein zweidimensionales Integral. Bauingenieure berechnen damit Erdaushub, Spieleentwickler die Datenmenge für Terrain-Meshes.',
      },
    },
    mistakes: {
      rotation_no_square: {
  ...
  '12-analytische-geometrie': {
    ...
    concepts: {
      plane_equation: {
        ...
        desc: 'Die kompakteste Darstellung einer Ebene: $n_1 x_1 + n_2 x_2 + n_3 x_3 = d$. Die Koeffizienten $n_1, n_2, n_3$ sind die Komponenten des Normalenvektors, $d$ ergibt sich durch Einsetzen eines bekannten Ebenenpunkts. Jeder Punkt $(x_1, x_2, x_3)$, der die Gleichung erfüllt, liegt in der Ebene.',
      },
      cross_product: {
        title: 'Kreuzprodukt',
        desc: 'Das Kreuzprodukt zweier Vektoren $\\vec{u}$ und $\\vec{v}$ liefert einen neuen Vektor $\\vec{n}$, der senkrecht auf beiden steht. Sein Betrag $|\\vec{u} \\times \\vec{v}|$ entspricht der Fläche des aufgespannten Parallelogramms. Wichtig: $\\vec{u} \\times \\vec{v} = -(\\vec{v} \\times \\vec{u})$ — die Reihenfolge bestimmt die Richtung!',
      },
      distance_formula: {
    ...
    examples: {
      game_world_plane: {
        ...
        kai_comment: 'In der Engine passiert genau das für jedes Dreieck: drei Vertices rein, Kreuzprodukt berechnen, Ebene gespeichert. Der Normalenvektor bestimmt gleichzeitig, in welche Richtung die Fläche "schaut" — entscheidend für Beleuchtung und Backface Culling!',
      },
      collision_detection: {
        ...
        step3: 'Nach $t$ auflösen — zusammenfassen und vereinfachen:',
        step4: 'Schnittpunkt berechnen — $t = 0$ in die Geradengleichung einsetzen:',
        kai_comment: 'Bei $t = 0$ trifft die Gerade sofort die Ebene — das Geschoss startet also direkt an der Wand. In der Engine prüfe ich zusätzlich $t > 0$ (Vorwärtsrichtung) und $0 \\leq t \\leq t_{\\max}$ (Reichweite). So filtere ich Treffer hinter dem Spieler oder ausserhalb der Reichweite raus.',
      },
    },
    realworld: {
      '3d_rendering': {
        title: '3D-Rendering & Beleuchtung',
        desc: 'Jede 3D-Engine berechnet Beleuchtung über das Skalarprodukt von Normalenvektor und Lichtrichtung: $\\vec{n} \\cdot \\vec{l} = \\cos(\\theta)$. Je kleiner der Winkel zwischen Licht und Flächennormale, desto heller die Fläche. Das ist das Lambert-Beleuchtungsmodell — die Grundlage für realistisches Licht in Games, Filmen und AR-Apps.',
      },
      architecture: {
        title: 'Architektur & BIM-Software',
        desc: 'In Architekturprogrammen wie AutoCAD oder Revit werden Dachflächen, Wände und Rampen als Ebenen modelliert. Die Koordinatenform beschreibt die Lage im Raum, der Normalenvektor zeigt die Ausrichtung. Statiker berechnen mit Ebenengleichungen, wie Kräfte auf schräge Flächen wirken — vom Dach bis zur Brückenfahrbahn.',
      },
      gps_navigation: {
        title: 'GPS & Drohnennavigation',
        desc: 'GPS-Koordinaten sind dreidimensional (Länge, Breite, Höhe). Drohnen navigieren entlang von Geraden im 3D-Raum und müssen Hindernisse — modelliert als Ebenen — erkennen. Der Abstand Punkt-Ebene entscheidet, ob eine Drohne einer Hausfassade zu nahe kommt. Dieselbe Mathematik nutzen selbstfahrende Autos für die LIDAR-Auswertung.',
      },
    },
    mistakes: {
      cross_product_order: {
  ...
  '12-hypothesentests': {
    ...
    concepts: {
      ...
      error_types: {
        ...
        desc: 'Fehler 1. Art ($\\alpha$): $H_0$ wird abgelehnt, obwohl sie wahr ist — ein "Fehlalarm". Fehler 2. Art ($\\beta$): $H_0$ wird beibehalten, obwohl $H_1$ wahr ist — ein "verpasster Effekt". Bei festem $n$ sind $\\alpha$ und $\\beta$ gegenläufig: Kleineres $\\alpha$ bedeutet größeres $\\beta$. Abhilfe: Größerer Stichprobenumfang $n$.',
      },
    },
    examples: {
      playtest_balance: {
    ...
    realworld: {
      ab_testing: {
        ...
        desc: 'Jedes Mal wenn Netflix den "Jetzt abspielen"-Button ändert oder Spotify eine neue Playlist-Ansicht testet, läuft ein Hypothesentest. Version A gegen Version B: Welche hat mehr Klicks? Der $p$-Wert entscheidet, ob der Unterschied echt ist oder nur zufällige Schwankung. Auch TikTok, YouTube und Instagram nutzen A/B-Tests — jedes Feature, das du siehst, hat einen Signifikanztest bestanden.',
      },
      quality_assurance: {
        title: 'Qualitätskontrolle in der Produktion',
        desc: 'In Fabriken wird ständig getestet: Liegt die Ausschussrate unter dem Grenzwert? Entspricht das Füllgewicht der Chipstüte dem aufgedruckten Wert? Hypothesentests schützen Verbraucher — und Unternehmen vor teuren Rückrufaktionen. Bei jeder Stichprobenprüfung steckt dieselbe Mathematik dahinter, die ihr hier lernt.',
      },
      clinical_trials: {
        title: 'Klinische Studien & Medikamente',
        desc: 'Bevor ein Medikament zugelassen wird, muss es einen Hypothesentest bestehen: $H_0$ sagt "Das Medikament wirkt nicht besser als ein Placebo". Nur wenn der $p$-Wert unter $\\alpha = 0{,}05$ fällt, gilt die Wirksamkeit als nachgewiesen. Die strengen Regeln ($\\alpha = 0{,}05$, $\\beta \\leq 0{,}20$) schützen Patienten vor unwirksamen Medikamenten — aber auch vor zu frühen Ablehnungen wirksamer Therapien.',
      },
    },
    mistakes: {
      accept_h0: {
